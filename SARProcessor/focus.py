...
class RD:
    ...
    def calculate_axes(self):
        ...
        self.az_freq_vals = np.arange(-self.az_sample_freq / 2, self.az_sample_freq / 2,
    ...
    def calculate_spacecraft_velocity(self):
        ...
        self.x_interp = interp1d(self.ephemeris["POD Solution Data Timestamp"].unique(),
                                 self.ephemeris["X-axis position ECEF"].unique(),
                                 fill_value="extrapolate")
        self.y_interp = interp1d(self.ephemeris["POD Solution Data Timestamp"].unique(),
                                 self.ephemeris["Y-axis position ECEF"].unique(),
                                 fill_value="extrapolate")
        self.z_interp = interp1d(self.ephemeris["POD Solution Data Timestamp"].unique(),
                                 self.ephemeris["Z-axis position ECEF"].unique(),
                                 fill_value="extrapolate")
        self.space_velocities = self.selection.apply(lambda x: velocity_interp(x["Coarse Time"] + x["Fine Time"]),
                                                     axis=1)

    def calculate_positions(self):
        """Calculate x, y, and z positions for each azimuth line."""
        self.x_positions = self.selection.apply(lambda x: self.x_interp(x["Coarse Time"] + x["Fine Time"]),
                                                axis=1).to_list()
        self.y_positions = self.selection.apply(lambda x: self.y_interp(x["Coarse Time"] + x["Fine Time"]),
                                                axis=1).to_list()
        self.z_positions = self.selection.apply(lambda x: self.z_interp(x["Coarse Time"] + x["Fine Time"]),
                                                axis=1).to_list()
    ...
    def apply_src_filter(self):

        Kr = self.range_sample_freq

        K_src = np.zeros((self.len_az_line, self.len_range_line))

        for i in range(self.len_az_line):
            for j in range(self.len_range_line):
                K_src[i, j] = (2 * self.velocities[i, j] ** 2 * self.f0 ** 3 * self.D[i, j] ** 2) / (
                        self.c * self.slant_range[j] * self.az_freq_vals[i])

        Km = Kr / (1 - (Kr / K_src))

        range_filter_H_src = np.zeros((self.len_az_line, self.len_range_line), dtype=complex)

        for i in range(self.len_az_line):
            for j in range(self.len_range_line):
                range_filter_H_src[i, j] = cmath.exp(2j * cmath.pi * self.range_freq_vals[j] ** 2 / (K_src[i, j]))

        for az_index in range(self.len_az_line):
            self.freq_domain_data[az_index, :] = self.freq_domain_data[az_index, :] * range_filter_H_src[az_index, :]
    ...
    def process(self):
        ...
        print("Step 12.1/14: Applying SRC filter...")
        self.apply_src_filter()
...
#         self.apply_range_filter()
#         logger.info("Step 13/14: Applying SRC filter...")
#         self.apply_src_filter()
