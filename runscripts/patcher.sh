...
python -m SARProcessor.patchy 0 $product_idx
python -m SARProcessor.patchy 1 $product_idx
python -m SARProcessor.patchy 2 $product_idx
python -m SARProcessor.patchy 3 $product_idx
python -m SARProcessor.patchy 4 $product_idx
python -m SARProcessor.patchy 5 $product_idx
python -m SARProcessor.patchy 6 $product_idx
python -m SARProcessor.patchy 7 $product_idx
python -m SARProcessor.patchy 8 $product_idx
python -m SARProcessor.patchy 9 $product_idx
...
#     num_chunks=10
#     for idx_chunk in $(seq 1 $num_chunks); do
#         python -m SARProcessor.patchy 
#         # python -m SARProcessor.patchy --data $product --meta $meta --ephemeris $ephemeris --output $OUTPUT_DIR --idx_chunk $idx_chunk
#     done
# done
# echo "========= End Patching ========="
